...
export type PersonSkeletonProps = { data?: PersonEntity };
export const PersonSkeleton = ({ data }: PersonSkeletonProps) => (
  <Container
    overflow="hidden"
    alignContent="center"
    alignItems="center"
...
export const ListSkeleton = ({ persons }: ListSkeletonProps) => (
  <Container
    autoFlow="dense"
  ...
  >
    {persons ? (
      persons.map((person) => (
        <PersonSkeleton key={person.uuid} data={person} />
      ))
    ) : (
      <>
        <PersonSkeleton />
